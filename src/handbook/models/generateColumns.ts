...
export const initialColumns = (): Array<Column<HomeSwordsTableContents>> => {
  ...
  const columns: Array<Column<HomeSwordsTableContents>> = [
    ...
    { title: '入手日', field: 'created_at', filtering: false, cellStyle: { ...numberCellStyle, background: 'ivory' } },
  ];
  ...
  Object.entries(statusType).forEach(([key, value]) => {
    if (
      ...
      if (displayedStatus[value]) {
        // ステータスを表示する場合
        if (columns.every((column: Column<HomeSwordsTableContents>) => {
          return statusType[value] !== column.field;
        })) {
          columns.push(setStatusColumn(value));
        }
      } else {
        // ステータスを非表示にする場合
        columns.some(
          (column: Column<HomeSwordsTableContents>, index: number) => {
