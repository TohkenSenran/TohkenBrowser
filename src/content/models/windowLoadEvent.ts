...
export const windowLoadEvent = () => {
    ...
    chrome.storage.local.get('rootState', (response) => {
        ...
        store.dispatch(loadBrowserState(
            storageState.browserSetting ?
                storageState.browserSetting :
                rootState.browserSetting,
        ));
        store.dispatch(updateJsonState(
            storageState.responseJson ?
                storageState.responseJson :
                rootState.responseJson,
        ));
        store.dispatch(loadPartyPanelState(
            storageState.partyPanel ?
                storageState.partyPanel :
                rootState.partyPanel,
        ));
    });
};
