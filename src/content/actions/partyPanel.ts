...
export const selectDisplayText = (
  ...
  const getNextType = (oldType: statusType): statusType => {
    return oldType + 1 < Object.keys(statusType).length / 2 ? oldType + 1 : statusType.none;
  };
...
export const setEnableExtendView = (enableExtendView: boolean): SetEnableExtendViewAction => ({
  type: partyPanelActionType.SET_ENABLEEXTENDVIEW,
  enableExtendView,
});

export const setSelectViewStatus = (displayedStatus: boolean[]): SelectDisplayedStatusAction => ({
  type: partyPanelActionType.SELECT_DISPLAYEDSTATUS,
  displayedStatus,
});
